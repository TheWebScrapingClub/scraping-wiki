...
- [JavaScript Proxy tracing](concepts/javascript-proxy-tracing.md) — Run a challenge script with a Proxy standing in for window so every property read, in-check and call is logged, including names that do not exist. Enumerates what a VM-obfuscated anti-bot reads without reversing it. Valid only when tapped and untapped tokens match byte for byte. Complementary to camoufox-reverse engine-level tracing.
...
- [VM-based JavaScript obfuscation](concepts/vm-based-javascript-obfuscation.md) — Detection logic shipped as an encrypted bytecode program run by a JavaScript interpreter, with runtime keys, per-load opcode permutation, integrity self-checks and builds that rotate every couple of weeks. Google SearchGuard is the documented instance; single-property patches last one build.
