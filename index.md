...
- [canvas-fingerprinting](concepts/canvas-fingerprinting.md) — A high-entropy fingerprinting signal that hashes off-screen canvas pixels, which vary by GPU, driver, and font rasterizer. Covers the randomization defenses, the WWW'25 Pixel-Recovery attack (Nguyen and Vadrevu) that reverses position-seeded noise, content-aware noise as the counter (LeooNic Camoufox patch, Brave Farbling), and our PropertyTracer observation of Datadome reading toDataURL/getImageData on leboncoin.
...
- [webrtc-ip-leak](concepts/webrtc-ip-leak.md) — WebRTC's ICE candidate gathering can reveal a client's real LAN/WAN IP to a page even behind a proxy, because STUN runs over UDP that an HTTP proxy does not carry. A fingerprint-coherence signal. We found stock Camoufox 146 still leaks the real WAN IP via the reflexive candidate under an HTTP proxy; the JWriter20 fork suppresses it.
...
- [Camoufox vs its forks](comparisons/camoufox-vs-forks.md) — Stock Camoufox against its three active forks (camoufox-reverse, LeooNic, JWriter20), graded on what each changes and what survives Datadome on leboncoin. The counterintuitive result: the most-patched fork (JWriter20) is blocked 100% while stock Camoufox passes. Stock on a clean IP wins.
